...
public class PartListing {
	 ...
	 *	}
	 */
	public PartListing(Map<String,Object> jsonMap){
		if(jsonMap != null){
			bucketName = (String)jsonMap.get("Bucket");
			key = (String)jsonMap.get("Key");
			
			String userId = (String) jsonMap.get("Owner");
			if(userId!=null)
				this.initiator = new Owner(userId,"");
			
			@SuppressWarnings("unchecked")
			List<Map<String,Object>> partList = (List<Map<String, Object>>) jsonMap.get("Parts");
			if(partList != null){
				parts = new ArrayList<PartSummary>();
				for(Map<String,Object> map : partList){
					PartSummary partSummary = new PartSummary(map);
